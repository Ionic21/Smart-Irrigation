...
int lastThreshold = 50;  // Last received threshold (default)
...
void loop() {
  if (isInitiator) {
    ...
    if (!gotResponse) {
      Serial.println("No response from Home ESP. Using last known threshold.");
      evaluateAndControlValve(false, lastThreshold);
    }
...
void processIncoming(String incoming) {
  ...
  lastThreshold = receivedValue;  // Update cached threshold
